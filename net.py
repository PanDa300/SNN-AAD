...
class network(object):
    ...
    def __init__(self, ni, nh, no):
        ...
        self.sparse = get_0_1_array(self.input_n, self.hidden_n, 0.5)
        ...
        self.input_weights = np.multiply(self.input_weights, self.sparse)
...
def manual_label():
    ...
    for i in range(0, len(test_data_)):
        ...
        normal = 0
        for j in range(0, p):
            if net_list[j].predict(test_data_[i])[0] > 0.5:
                abnormal = abnormal + 1
            else:
                normal = normal + 1
